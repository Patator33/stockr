...
interface Variant { id: string; productId: string; name: string; costPrice: number; salePrice: number; shippingCost: number; barcode?: string | null; supplierRef?: string | null; stocks?: { quantity: number; location: { name: string } }[]; }
  ...
  return (
    <div>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1.5rem' }}>
        <h1 style={{ margin: 0, fontSize: '1.25rem', fontWeight: 800 }}>Gestion</h1>
      </div>
...
function ProductsTab() {
  ...
  const loadProducts = () => fetch('/api/products').then(r => r.json()).then(p => setProducts(Array.isArray(p) ? p : []));
  const loadVariants = (pid: string) => fetch(`/api/variants?productId=${pid}`).then(r => r.json()).then(v => setVariants(Array.isArray(v) ? v : []));

  useEffect(() => {
    loadProducts();
    fetch('/api/locations').then(r => r.json()).then(l => setLocations(Array.isArray(l) ? l : []));
  }, []);

  const selectProduct = (p: Product) => { setSelected(p); setEditProduct(null); setShowNewVariant(false); loadVariants(p.id); };

  const createProduct = async (e: React.FormEvent) => {
    e.preventDefault();
    await fetch('/api/products', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ name: newName, description: newDesc }) });
    setNewName(''); setNewDesc(''); setShowNewProduct(false);
    await loadProducts();
  };
  ...
  const updateProduct = async (e: React.FormEvent) => {
    ...
    await fetch(`/api/products/${editProduct.id}`, { method: 'PUT', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ name: editProduct.name, description: editProduct.description, defaultLocationId: editProduct.defaultLocationId }) });
    setSelected(editProduct); setEditProduct(null);
    await loadProducts();
  };
  ...
  const createVariant = async (e: React.FormEvent) => {
    ...
    if (!selected) return;
    setVError('');
    const res = await fetch('/api/variants', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ productId: selected.id, ...vForm, attributes: [], barcode: vForm.barcode || null, supplierRef: vForm.supplierRef || null }) });
    const d = await res.json();
    if (!res.ok) { setVError(d.error || 'Erreur'); return; }
    setVForm({ name: '', costPrice: 0, salePrice: 0, shippingCost: 0, barcode: '', supplierRef: '' }); setShowNewVariant(false);
  ...
  const deleteVariant = async (id: string) => {
    ...
    if (selected) await loadVariants(selected.id);
  };
    ...
    <div style={{ display: 'flex', gap: '1.5rem' }}>
      ...
      <div style={{ width: '18rem', flexShrink: 0 }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '0.75rem' }}>
          ...
          <button onClick={() => setShowNewProduct(!showNewProduct)} className="btn-ghost" style={{ fontSize: '0.75rem', padding: '0.25rem 0.625rem' }}>+</button>
        </div>
        {showNewProduct && (
        ...
        <div className="card" style={{ flex: 1 }}>
          {editProduct ? (
            <form onSubmit={updateProduct} style={{ display: 'flex', flexDirection: 'column', gap: '0.75rem', marginBottom: '1.5rem' }}>
              ...
              <div><label style={{ fontSize: '0.75rem', color: '#64748b', display: 'block', marginBottom: '0.25rem' }}>Nom</label><input value={editProduct.name} onChange={e => setEditProduct(p => p ? { ...p, name: e.target.value } : p)} required /></div>
              <div><label style={{ fontSize: '0.75rem', color: '#64748b', display: 'block', marginBottom: '0.25rem' }}>Description</label><input value={editProduct.description || ''} onChange={e => setEditProduct(p => p ? { ...p, description: e.target.value } : p)} /></div>
              <div>
                <label style={{ fontSize: '0.75rem', color: '#64748b', display: 'block', marginBottom: '0.25rem' }}>Zone par défaut</label>
                <select value={editProduct.defaultLocationId || ''} onChange={e => setEditProduct(p => p ? { ...p, defaultLocationId: e.target.value || null } : p)}>
              ...
              <div style={{ display: 'flex', gap: '0.5rem' }}>
                ...
                <button type="button" onClick={() => setEditProduct(null)} className="btn-ghost">Annuler</button>
              </div>
            </form>
          ) : (
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '1rem' }}>
              ...
              <div style={{ display: 'flex', gap: '0.5rem' }}>
                <button onClick={() => setEditProduct(selected)} className="btn-ghost" style={{ fontSize: '0.75rem' }}>Modifier</button>
          ...
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '0.75rem' }}>
            ...
            <button onClick={() => setShowNewVariant(!showNewVariant)} className="btn-ghost" style={{ fontSize: '0.75rem', padding: '0.25rem 0.625rem' }}>+</button>
          </div>
          ...
          {showNewVariant && (
            <form onSubmit={createVariant} style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: '0.5rem', marginBottom: '1rem', background: '#0f1629', padding: '0.75rem', borderRadius: '0.5rem' }}>
              ...
              <div><label style={{ fontSize: '0.7rem', color: '#64748b', display: 'block', marginBottom: '0.25rem' }}>Ref. fournisseur</label><input value={vForm.supplierRef} onChange={e => setVForm(f => ({ ...f, supplierRef: e.target.value }))} /></div>
              ...
              <div style={{ gridColumn: '1 / -1', display: 'flex', gap: '0.5rem' }}>
                ...
                <button type="button" onClick={() => setShowNewVariant(false)} className="btn-ghost" style={{ fontSize: '0.8125rem' }}>Annuler</button>
              </div>
            </form>
          )}
...
function StockTab() {
  ...
  const load = () => {
    setLoading(true);
    fetch('/api/stocks').then(r => r.json()).then(s => setStock(Array.isArray(s) ? s : [])).finally(() => setLoading(false));
  };
  ...
  const adjust = async (e: React.FormEvent) => {
    e.preventDefault();
    setMsg('');
    const res = await fetch('/api/stocks/adjust', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ variantId: form.variantId, locationId: form.locationId, quantity: Number(form.quantity) }) });
      ...
      {stock.map(s => (
        <div key={s.location.id} className="card">
          <h2 style={{ margin: '0 0 0.75rem', fontSize: '0.875rem', fontWeight: 700, color: '#94a3b8' }}>
            ...
            <span style={{ marginLeft: '0.5rem', color: '#475569', fontSize: '0.75rem', fontWeight: 400 }}>({s.items.length} références)</span>
          </h2>
          <table>
...
function LocationsTab() {
  ...
  const load = () => fetch('/api/locations').then(r => r.json()).then(l => setLocations(Array.isArray(l) ? l : []));
  useEffect(() => { load(); }, []);
  ...
  const save = async (id: string) => {
    await fetch(`/api/locations/${id}`, { method: 'PUT', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(editForm) });
    setEditId(null);
    load();
  };

  const setDefault = async (id: string) => {
    await fetch(`/api/locations/${id}`, { method: 'PATCH', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ isDefault: true }) });
    load();
  };
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: '1.5rem', maxWidth: '40rem' }}>
      <div className="card">
        ...
        <form onSubmit={create} style={{ display: 'flex', flexDirection: 'column', gap: '0.5rem' }}>
          <input value={form.name} onChange={e => setForm(f => ({ ...f, name: e.target.value }))} placeholder="Nom de la zone" required />
              ...
              {editId === l.id ? (
                <>
                  <div style={{ flex: 1, display: 'flex', gap: '0.5rem' }}>
                    <input value={editForm.name} onChange={e => setEditForm(f => ({ ...f, name: e.target.value }))} style={{ flex: 1 }} />
                    <input value={editForm.description} onChange={e => setEditForm(f => ({ ...f, description: e.target.value }))} placeholder="Description" style={{ flex: 1 }} />
                  </div>
                ...
                <>
                  <div style={{ flex: 1 }}>
                    <p style={{ margin: 0, fontWeight: 600, color: '#e2e8f0', fontSize: '0.875rem' }}>
                      {l.isDefault && <span style={{ color: '#f59e0b', marginRight: '0.375rem' }}>★</span>}{l.name}
                    </p>
                  ...
                  <button onClick={() => { setEditId(l.id); setEditForm({ name: l.name, description: l.description || '' }); }} className="btn-ghost" style={{ fontSize: '0.75rem', padding: '0.25rem 0.5rem' }}>Modifier</button>
...
function ReturnsTab() {
  ...
  const submit = async () => {
    ...
    if (items.length === 0) { setMsg('Aucun article à retourner'); return; }
    if (!locationId) { setMsg('Sélectionnez une zone'); return; }
    setLoading(true);
    const res = await fetch('/api/stock-returns', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ items, locationId }) });
    const d = await res.json();
    if (!res.ok) { setMsg(d.error || 'Erreur'); } else { setMsg(`✓ ${d.count} article(s) retournés en stock`); setQuantities({}); }
    setLoading(false);
  };
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '0.375rem', marginBottom: '1rem' }}>
          {variants.map(v => (
            <div key={v.id} style={{ display: 'flex', alignItems: 'center', gap: '0.75rem', padding: '0.5rem', borderRadius: '0.375rem', background: '#0f1629' }}>
              <span style={{ flex: 1, fontSize: '0.875rem', color: '#e2e8f0' }}>{v.name}</span>
              <div style={{ display: 'flex', alignItems: 'center', gap: '0.375rem' }}>
                <button onClick={() => setQuantities(q => ({ ...q, [v.id]: Math.max(0, (q[v.id] || 0) - 1) }))} style={{ width: '1.75rem', height: '1.75rem', background: '#1a2035', border: '1px solid #2a3045', borderRadius: '0.375rem', color: '#e2e8f0', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>−</button>
                ...
                <button onClick={() => setQuantities(q => ({ ...q, [v.id]: (q[v.id] || 0) + 1 }))} style={{ width: '1.75rem', height: '1.75rem', background: '#1a2035', border: '1px solid #2a3045', borderRadius: '0.375rem', color: '#e2e8f0', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>+</button>
              </div>
            </div>
          ))}
        </div>
